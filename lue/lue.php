...
/*
Plugin Name: lue
Plugin URI: http://precisionprojectmarketing.com
Description: Jared's customizations for WordPress
Version: 1.0.0
Author: Jared Mock
Author URI: http://precisionprojectmarketing.com

    Copyright (c) 2016 Jared Mock (http://precisionprojectmarketing.com)
    lue is released under the GNU General Public
...
function lue_enqueue_js() {
    ...
    wp_enqueue_script( 'lue-js--styling-adjustments',        plugin_dir_url( __FILE__ ) . 'js/styling-adjustments.js',      array( 'jquery' ), '1.1',                                     true);
    wp_enqueue_script( 'lue-js--mobile',                     plugin_dir_url( __FILE__ ) . 'js/mobile.js',                   array( 'jquery' ), '1.1',                                     true);
}
...
/**
 * Add the Student Info to the checkout
 */
add_action( 'woocommerce_student_name', 'student_name' );

function student_name( $checkout ) {

    echo '<div id="student_name"><h2>' . __('Student Information') . '</h2>';
...
/**
 * Process the checkout with additional information
 */
...
function my_custom_checkout_field_process() {
    ...
    if ( ! $_POST['student_name'] )
        wc_add_notice( __( 'Please enter the students name.' ), 'error' );
}
...
/* Fix part of the bugginess of the WP editor */
